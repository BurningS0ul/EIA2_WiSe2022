...
namespace Artpiece {
    ...
    function letItSnow(_position: Vector): void {
        let snowflakes: number = getRandomNumber();

        for (let flake: number = 0; flake < snowflakes; flake++) {
            let minrad: number = Math.round(Math.random() * 1);
            let maxrad: number = Math.round(Math.random() * 3);
            let xpos: number = Math.round(Math.random() * innerWidth);
            let ypos: number = Math.round(Math.random() * innerHeight);
            let rad: number = Math.floor(Math.random() * (maxrad - minrad + 1) + minrad);
            let gradient: CanvasGradient = ctx.createRadialGradient(xpos, ypos, maxrad, xpos + 40, ypos + 40, rad);
